...
| `FQDN внешней точки входа` | Имя, по которому пользователь открывает WAC. | Записывается в настройки шлюза. По умолчанию берется имя компьютера. |
| `FQDN внутренней службы` | Имя для внутренних обращений служб WAC. | Записывается в настройки службы. Обычно оставляют `localhost`. |
...
| `Отпечаток сертификата` | Thumbprint существующего TLS-сертификата. | Ищется в `LocalMachine\My`. Сертификат должен подходить для Server Authentication и иметь доступный закрытый ключ. |
| `Subject создаваемого сертификата` | Имя нового самоподписанного сертификата. | Используется только когда отпечаток не задан. По умолчанию `WindowsAdminCenterSelfSigned`. |
...
- создает или переиспользует сертификат `WindowsAdminCenterSelfSigned`;
...
- при необходимости включает WinRM поверх HTTPS и открывает порт `5986`;
...
Эти команды сохраняют `C:\ProgramData\WindowsAdminCenter` и передают новый `-CertificateSubject`, чтобы сценарий не переиспользовал старый сертификат.
